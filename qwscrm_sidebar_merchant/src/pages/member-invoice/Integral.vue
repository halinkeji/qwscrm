<template>
  <q-page class=" bg-grey-1">
    ...
    <q-header class="bg-white" bordered>
      <q-toolbar class="text-grey-8">
        ...
        <q-toolbar-title class="text-subtitle1 no-padding q-pt-xs">
          {{ $store.state.member.storeBaseConfig.consume_base_integralUnit }}记录
        </q-toolbar-title>
      </q-toolbar>
      <div class="row bg-white full-width">
        <q-expansion-item label="根据条件筛选" class="full-width text-dark">
          <q-input outlined bottom-slots v-model="searchFrom.searchSerm" @change="getPointsDetails()" dense class="col-12 q-pa-sm" label="单据号">
            <template v-slot:append>
              <q-icon name="search" @click="getPointsDetails()" />
            </template>
          </q-input>
                  ...
                  <q-popup-proxy transition-show="scale" transition-hide="scale">
                    <q-date v-model="created_at" minimal mask="YYYY-MM-DD HH:mm">
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="确定" @click="getPointsDetails" color="primary" flat />
                      </div>
                    </q-date>
                  </q-popup-proxy>
                </q-icon>
    ...
    <div class="q-px-md q-pt-md ful-height">
      <div v-for="(item, index) in pointsDetailsList">
        <q-item clickable>
          <q-item-section avatar style="margin-bottom: 6%">
            <q-avatar size="md" color="orange-6" text-color="white" icon="las la-cash-register" />
          </q-item-section>

          <q-item-section>
            <q-item-label v-if="!item.sc_invoiceCode">{{ item.pcr_invoiceCode }}</q-item-label>
            <q-item-label v-else>{{ item.sc_invoiceCode }}</q-item-label>
            <q-item-label caption lines="2">{{ item.pcr_operatingBehavior }}</q-item-label>
            <q-item-label caption lines="2">{{ $q_date.formatDate(item.created_at * 1000, 'YYYY-MM-DD  HH:mm') }}</q-item-label>
          </q-item-section>

          <q-item-section side style="margin-bottom: 8%">
            <q-item-label caption style="font-size: 15px; color: black">
              <div v-if="item.pcr_pointsCountStatus == 1">+ {{ item.pcr_pointsCount }}</div>
              <div v-else>-{{ item.pcr_pointsCount }}</div>
            </q-item-label>
          </q-item-section>
        </q-item>
        <q-separator inset />
    ...
    getPointsDetails () {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            if (res.data.list) {
              this.pointsDetailsList = res.data.list ? res.data.list : []
