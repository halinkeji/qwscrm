<template>
  <q-page class="bg-color-EAE full-width column no-wrap" :style="cssVars">
    ...
    <q-page-sticky position="bottom" :offset="[0, 18]" v-if=" !member_id">
      <q-btn unelevated rounded class="full-width" @click="inviteBind" color="deep-orange" label="邀请会员激活" />
    </q-page-sticky>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    ...
    onCheckQuery () {
      ...
      if (!this.$q.localStorage.getItem('qwscrm_merchant_header_local')) {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.code == 200 && res.data) {
            ...
            this.$store.commit('member/SET_CDN_URL', res.data.cdn_url)
            if (res.data && res.data.relationMember) {
              this.$store.commit('oauth/SET_MEMBER_RELATION', res.data.relationMember)
              this.member_id = res.data.relationMember.member_id
              this.getMemberInfo()
            }
    ...
    // end 设置关联客户
    inviteBind () {
      const headerLocal = LocalStorage.getItem('qwscrm_merchant_header_local')
      const userInfo = LocalStorage.getItem('qwscrm_merchant_user_info')

      const that = this
      sendChatMessage('news', {
        link: window.location.origin + '/m/qwscrm/merchant/bind-member?corp_id=' + headerLocal.corp_id + '&contactUserId=' + this.contactUserId, // H5消息页面url 必填
    ...
    getMemberInfo () {
      this.$store
        ...
        .then((res) => {
          if (res.code == 200) {
            this.$store.commit('member/SET_MEMBER_INFO', res.data)
          } else {
            this.$q.notify({
              message: '失败',
              icon: 'ion-close-circle-outline',
