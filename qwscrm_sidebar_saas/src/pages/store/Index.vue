<template>
  <q-page class="bg-color-EAE full-width column no-wrap">
    <q-card flat bordered v-if="showRegPage">
      <q-card-section>
    ...
    <div class="q-mx-xs q-mt-md q-mb-xl" >
      <q-card class="shadow-box q-mb-sm" v-if="storeInfo && storeInfo.id">
          ...
          <q-list bordered separator>
            ...
            <q-item clickable v-ripple>
              ...
              <q-item-section>
                <q-item-label
                  >状态
                  <q-btn outline dense color="positive" size="sm" class="q-ml-sm" v-if="parseInt(storeInfo.status) == 1">启用</q-btn>
      ...
      <q-card class="shadow-box q-mb-sm" v-if="libStoreList && libStoreList.length > 1">
        <q-card-section class="q-pa-none">
          <q-expansion-item default-opened>
            <template v-slot:header>
              ...
              <q-item-section> 商家门店列表 </q-item-section>
      ...
      <q-card class="shadow-box q-mb-sm" v-if="userList && userList.length > 0">
        <q-card-section class="q-pa-none">
          <q-expansion-item default-opened>
            <template v-slot:header>
              <q-item-section avatar>
              ...
              <q-item-section> 商家账号列表 </q-item-section>
            ...
            <q-card>
              <q-card-section class="q-pa-xs">
                <q-list bordered separator>
                  <q-item clickable v-ripple :key="'user' + key" v-for="(item, key) in userList">
                    <q-item-section>
      ...
      <q-card class="shadow-box q-mb-sm" v-if="applyLogList && applyLogList.length > 0">
        <q-card-section class="q-pa-none">
          <q-expansion-item default-opened>
            <template v-slot:header>
              <q-item-section avatar>
...
import BindWorker from '../components/BindWorker.vue'
...
export default {
  ...
  mounted () {
    ...
    } else if (this.$store.state.oauth.qwUserInfo && this.$store.state.oauth.qwUserInfo.relationUser) {
  ...
  methods: {
    getData () {
      ...
      const relationUser = this.$store.state.oauth.qwUserInfo && this.$store.state.oauth.qwUserInfo.relationUser ? this.$store.state.oauth.qwUserInfo.relationUser : {}

      this.$store
        .dispatch('storeManage/getData', { user_id: relationUser.relation_user_id, contact_id: this.contact_id })
        .then((res) => {
          if (res.code == 200) {
    ...
    bindUserRelation () {
      this.$refs.regForm.validate().then((success) => {
        if (success && this.$store.state.oauth.qw_external_userid) {
          const obj = {
            regMobile: this.regMobile,
            qw_external_userid: this.$store.state.oauth.qw_external_userid
          }
          this.$store
