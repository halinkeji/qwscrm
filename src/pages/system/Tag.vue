        ...
        <div class="row q-col-gutter-x-md q-col-gutter-y-xs full-width">
          <div class="col-auto q-pl-none q-gutter-sm no-wrap">
            ...
            <q-btn
              ...
              @click="getItemData(selected[0].id)"
            />
            <q-btn
              dense
              color="primary"
...
export default {
  ...
  methods: {
    selectedTable(){
      let selected = this.tagDataList.map(item => {
            return item.id
        })
    ...
    refreshData(){
       this.loading = true
       this.$store
        .dispatch("workerTag/refreshData")
        .then(res => {
          if (res.code == 200) {
             //最后同步时间
            this.syncDate = res.data.sync_date ? date.formatDate(res.data.sync_date * 1000, 'YYYY-MM-DD  HH:mm:ss') : '还未同步'
    ...
    editData(){
      ...
      this.$store
        .dispatch("workerTag/editData",item)
        .then(res => {
            ...
            if (res.code == 200) {
              ...
              this.$q.notify({
                message: "失败",
                caption: `${this.dialogForm.id ? '修改' : '新增'}失败，请联系管理员1`,
                icon: "ion-close-circle-outline",
                color: "red",
                timeout: 500,
                position: "top-right"
          ...
          .catch(error => {
            ...
            this.$q.notify({
                message: "失败",
                caption: `${this.dialogForm.id ? '修改' : '新增'}失败，请联系管理员2`,
                icon: "ion-close-circle-outline",
                color: "red",
                timeout: 500,
                position: "top-right"
    ...
    deleteData(id){
      ...
      this.loading = true
      this.$store.dispatch("workerTag/deleteData",{delIds})
          .then(res => {
    ...
    getItemData(id){
      ...
      this.$store
        .dispatch("workerTag/getItem", payload)
        .then(res => {
          if (res.code == 200 && res.data) { 
    ...
    viewItemToList(id){
      ...
      this.$store.dispatch("workerTag/getItemToList", payload)
        .then(res => {
          if (res.code == 200 && res.data) { 
