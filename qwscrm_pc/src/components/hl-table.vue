...
export default {
  ...
  props: {
    ...
    selectedRowsLabel: {
      type: Function,
      default: (numberOfRows) =>  ' 已选 '  + numberOfRows + ' 条'
    },
  ...
  created () {
    ...
    if (!this.columns || this.columns.length === 0) {
      console.error('*** hl-table tree-table mode must set columns!!')
    }
  },
  watch: {
    expanded (valNew) {
  ...
  computed: {
    ...
    computedClass () {
      ...
      if (this.stickyHeader && this.stickyFirstColumn && this.stickyLastColumn) {
        cls += ' hl-table-sticky-header-and-first-last-column'
      } else if (this.stickyHeader && this.stickyFirstColumn) {
        cls += ' hl-table-sticky-header-and-first-column'
      } else if (this.stickyHeader && this.stickyLastColumn) {
        cls += ' hl-table-sticky-header-and-last-column'
      } else if (this.stickyFirstColumn && this.stickyLastColumn) {
        cls += ' hl-table-sticky-first-and-last-column'
      } else if (this.stickyHeader) {
        cls += ' hl-table-sticky-header'
      } else if (this.stickyFirstColumn) {
        cls += ' hl-table-sticky-first-column'
      } else if (this.stickyLastColumn) {
        cls += ' hl-table-sticky-last-column'
      }
      return cls
    },
    computedStyle () {
